...
export class Detail3Page {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public loadingCtrl: LoadingController, public database:AngularFireDatabase,public ref: ChangeDetectorRef,private toastCtrl: ToastController) {
      ...
      this.item = database.object('/list/'+this.itemId );

      this.itemOption = database.list('/list/'+this.itemId+'/options' );
      this.itemOption.subscribe(itemOption => {
          this.itemOptionArray = itemOption; 
      })

      this.itemSize = database.list('/list/'+this.itemId+'/size' );
